...
my.data <- read.csv("Class4.csv", sep = ",", dec = ".", header = T) 	# daily data
names(my.data)
head(my.data,10)
...
p2 + labs(x = "Time", y = "Noise Free Signal")	
...
# Setup output matrices for the state prediction and variances
...
# Initialize the state means and covar at t = 0
...
for (ii in 1:nn) {
	...
	at.tm1 <- tt %*% atm1 + d 									# prior mean
	pt.tm1 <- tt %*% ptm1 %*% t(tt) + qq 						# prior covar


	yhat <- zz %*% at.tm1 + c 									# predicted observed value
	err <- y2[ii] - yhat 										# prediction error 
	f <- zz %*% pt.tm1 %*% t(zz) + hh 							# variance in observation error
	kgain <- pt.tm1 %*% t(zz) %*% solve(f) 						# Kalman Gain Factor


	# INFORMATION UPDATE (based on observed data)
	at <- at.tm1 + kgain %*% err 								# posterior mean
	pt <- pt.tm1 - kgain %*% zz %*% pt.tm1 						# posterior covar
...
#### Plot Actual and Estimated Signal Overlaid by 95% CI #####
...
plot(out$days, out$M, ylim = c(min(out$L), max(out$U)), type = "l", xlab = "Days", ylab = "Estimated Signal", main = "Kalman Filter State Estimates with 95% CI")
